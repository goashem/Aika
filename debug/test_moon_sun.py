...
GROUND_TRUTH_VALUES = {'date_time': '2026-01-09 14:16:00', 'location': 'Puistokatu 12, Turku, Finland',
                       'coordinates': {'latitude': 60.4477801, 'longitude': 22.2503515},
                       'sun': {'altitude': 5.6, 'azimuth': 202.7, 'dawn_start': '08:34', 'sunrise': '09:30', 'solar_noon': '12:37', 'sunset': '15:45',
                               'dusk_end': '16:41'},
                       'moon': {'altitude': -23.6, 'azimuth': 304.8, 'illumination_percent': 61.3, 'rise_time': '00:43', 'peak_time': '05:51',
                                'set_time': '10:59'}}

# Test tolerance (acceptable error margins)
TEST_TOLERANCE = {'sun_altitude': 0.5,  # degrees
...
def validate_sun_position(calculated_altitude, calculated_azimuth):
    ...
    return {'altitude': {'expected': expected_alt, 'calculated': calculated_altitude, 'difference': alt_diff, 'pass': alt_pass},
            'azimuth': {'expected': expected_az, 'calculated': calculated_azimuth, 'difference': az_diff, 'pass': az_pass},
...
def validate_moon_position(calculated_altitude, calculated_azimuth, calculated_illumination):
    ...
    return {'altitude': {'expected': expected_alt, 'calculated': calculated_altitude, 'difference': alt_diff, 'pass': alt_pass},
            'azimuth': {'expected': expected_az, 'calculated': calculated_azimuth, 'difference': az_diff, 'pass': az_pass},
            'illumination': {'expected': expected_illum, 'calculated': calculated_illumination, 'difference': illum_diff, 'pass': illum_pass},
