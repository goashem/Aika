...
from ..models import (
    SolarInfo, DaylightInfo, GoldenBlueHours, SunCountdown,
    LunarInfo, EclipseInfo
)
...
def get_daylight_info(latitude, longitude, now, timezone):
    ...
    return DaylightInfo(
        daylight_hours=data.get("daylight_hours", 0.0),
        daylight_minutes=data.get("daylight_minutes", 0),
        change_from_yesterday=data.get("change_from_yesterday", 0),
        change_direction=data.get("change_direction", "same")
    )
...
def get_golden_blue_hours(latitude, longitude, now, timezone):
    ...
    return GoldenBlueHours(
        morning_golden_hour=data.get("morning_golden_hour"),
        evening_golden_hour=data.get("evening_golden_hour"),
        morning_blue_hour=data.get("morning_blue_hour"),
        evening_blue_hour=data.get("evening_blue_hour"),
        is_golden_hour_now=data.get("is_golden_hour_now", False),
        is_blue_hour_now=data.get("is_blue_hour_now", False)
    )
...
def get_sun_countdown(latitude, longitude, now, timezone):
    ...
    return SunCountdown(
        time_to_sunrise=data.get("time_to_sunrise"),
        time_to_sunset=data.get("time_to_sunset"),
        sun_is_up=data.get("sun_is_up", False),
        next_event=data.get("next_event"),
        next_event_time=data.get("next_event_time"),
...
def get_lunar_info(latitude, longitude, now, timezone, translations):
    ...
    return LunarInfo(
        ...
        special_phase=data.get("special_phase"),
