...
def get_road_weather(latitude, longitude, country_code):
    """Wrapper for providers.road.get_road_weather."""
    return road_provider.get_road_weather(latitude, longitude, country_code)


def get_electricity_price(now, timezone, country_code):
    """Wrapper for providers.electricity.get_electricity_price."""
    return electricity_provider.get_electricity_price(now, timezone, country_code)


def get_detailed_electricity_pricing(now, timezone, country_code):
    """Wrapper for providers.electricity.get_detailed_electricity_pricing."""
    return electricity_provider.get_detailed_electricity_pricing(now, timezone, country_code)


def get_aurora_forecast():
    """Wrapper for providers.aurora.get_aurora_forecast."""
    return aurora_provider.get_aurora_forecast()


# Finnish city bounding boxes mapped to Digitransit feed names
# Format: (min_lat, max_lat, min_lon, max_lon, feed_name, router)
FINNISH_CITY_FEEDS = transit_provider.FINNISH_CITY_FEEDS


def get_city_feed(latitude, longitude):
    """Wrapper for providers.transit.get_city_feed."""
    return transit_provider.get_city_feed(latitude, longitude)


def is_in_foli_area(latitude, longitude):
    """Wrapper for providers.transit.is_in_foli_area."""
    return transit_provider.is_in_foli_area(latitude, longitude)


def get_digitransit_alerts(now, feed_name, router, digitransit_api_key):
    """Wrapper for providers.transit.get_digitransit_alerts."""
    return transit_provider.get_digitransit_alerts(now, feed_name, router, digitransit_api_key)


def get_foli_alerts(now, digitransit_api_key=None):
    """Wrapper for providers.transit.get_foli_alerts."""
    return transit_provider.get_foli_alerts(now, digitransit_api_key)


def get_city_alerts(now, feed_name, router, digitransit_api_key):
    """Wrapper for providers.transit.get_city_alerts."""
    return transit_provider.get_city_alerts(now, feed_name, router, digitransit_api_key)


def get_transport_disruptions(latitude, longitude, now, country_code, digitransit_api_key):
    """Wrapper for providers.transit.get_transport_disruptions."""
    return transit_provider.get_transport_disruptions(latitude, longitude, now, country_code, digitransit_api_key)
